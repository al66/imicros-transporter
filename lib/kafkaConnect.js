...
class KafkaConnect {
    ...
    constructor({ transporter }) {
        ...
        this.maxBatchSize = this.opts.maxBatchSize || 2000;       // default 1000 messages (with awaitSend = false !)
    ...
    async subscribe(event, group, topic) {
            ...
            consumer.run({
                eachMessage: async ({ topic, partition, message }) => {
                    ...
                    return new Promise(((resolve) => {
                        setImmediate(async () => {
                            await this.transporter.receive(PACKET_EVENT, message.value.toString());
                            resolve();
                        });
                    }).bind(this));
                }
    ...
    async getTopics () {
        try {
            ...
            this.logger.info("Topics", this.topics);
            return existing;
        } catch (err) {
    ...
    async send(topic, data, { balanced, packet }) { 
        ...
        if (!this.topics.includes(kafkaTopic)) {
            if (!this.requestedTopics.includes(kafkaTopic)) {
                this.requestedTopics.push(kafkaTopic);
                this.checkTopics({ topics: [ kafkaTopic ] });
            }
        }
        
        let received = Date.now();
        let p = new Promise(resolve => this.commit.push({ timestamp: received, topic: kafkaTopic, msg: data, resolve }));
        
        /*
        if (this.commit.length >= this.maxBatchSize && this.topics.includes(kafkaTopic)) {
            this.sendBatch();
        } else {
            this.timers.push(setTimeout(function () { this.sendBatch({ last: received }); }.bind(this), this.timeToFlush));
        }
        */
    ...
    async sendBatch ({ flush = false } = {}) {
        if (!this.producer || this.disconnecting || this.disconnected) return this.batchRunner ? clearTimeout(this.batchRunner) : null ;
